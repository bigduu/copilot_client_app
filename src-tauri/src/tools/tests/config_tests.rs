...
#[cfg(test)]
mod tests {
    use crate::tools::config_manager::{ToolConfigManager, ConfigManagerBuilder};
    ...
    fn test_category_management() {
        ...
        assert_eq!(categories[0].name, "测试类别");
    }
    ...
    fn test_config_update() {
        ...
        config_manager.update_tool_config("update_tool", updated_config).unwrap();
    ...
    fn test_display_names() {
        ...
        assert_eq!(
            config_manager.get_tool_display_name("display_tool").unwrap(),
            "显示工具"
        );
        assert_eq!(
            config_manager.get_category_display_name("display_category").unwrap(),
            "显示类别"
        );
        
        // 测试不存在的项目
        assert!(config_manager.get_tool_display_name("nonexistent").is_none());
        assert!(config_manager.get_category_display_name("nonexistent").is_none());
    }
}