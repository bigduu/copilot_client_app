...
### 核心特性
- **Category trait 架构**：基于 [`Category`](category.rs:8) trait 的简洁类别系统
...
**[`Category`](category.rs:8) trait**
- 定义工具类别的标准接口
- 每个工具类别都必须实现此 trait
- 提供类别信息、工具配置、权限控制等功能

**[`CategoryInfo`](category.rs:22)**
- 包含完整的类别信息和工具配置
- 用于向前端提供统一的数据结构
...
在 [`categories/mod.rs`](categories/mod.rs) 中添加：
...
在 [`mod.rs`](mod.rs) 的 `get_available_categories()` 函数中添加：
