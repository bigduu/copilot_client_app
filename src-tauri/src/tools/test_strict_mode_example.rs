...
mod tests {
    ...
    fn demo_strict_tools_mode_usage() {
        ...
        assert!(cmd_category.strict_tools_mode, "命令执行类别应该启用严格模式");
        ...
        assert!(!file_category.strict_tools_mode, "文件操作类别不应该启用严格模式");
    ...
    fn demo_new_tool_category_strict_mode() {
        ...
        println!("=== NewToolCategory 严格模式演示 ===");

        // 创建一个自定义类别并设置严格模式
        let custom_category = NewToolCategory::new(
            "custom_ai".to_string(),
            "AI助手".to_string(),
        ...
        println!("✅ NewToolCategory 严格模式设置成功！");
    }
}