...
copilot_client_app 当前提供 MCP 工具调用和 Workflow 用户显式操作，但缺乏一个中间层来编排和组织这些能力。Skill 系统作为能力编排层，可以：

1. **打包相关能力**：将 MCP 工具、Workflow、提示词片段组合成可复用的技能单元
...
- 新增 SkillEditor：创建/编辑 Skill，关联 MCP 工具和 Workflow
...
- **No breaking changes**: 现有 Chat/MCP/Workflow API 保持不变
- **Dependencies**: 复用现有 MCP 工具和 Workflow 系统
