...
use bamboo_core::{Message, tools::ToolSchema, tools::ToolCall, tools::FunctionCall};
...
impl CopilotProvider {
    ...
    pub fn with_token(token: impl Into<String>) -> Self {
        let mut provider = Self::with_client(Self::build_client());
        provider.token = Some(token.into());
        provider
    }
    ...
    pub async fn authenticate(&mut self) -> std::result::Result<(), LLMError> {
        ...
        if let Some(cache) = TokenCache::load().await {
            if cache.is_valid() {
                ...
                log::info!("Using cached Copilot token (expires in {} minutes)", remaining / 60);
    ...
    fn build_headers(&self) -> std::result::Result<reqwest::header::HeaderMap, LLMError> {
        ...
        headers.insert("user-agent", HeaderValue::from_static("GitHubCopilotChat/0.20.3"));
    ...
    fn convert_messages(&self, messages: &[Message]) -> Vec<serde_json::Value> {
        messages.iter().map(|m| {
            let role = match m.role {
                bamboo_core::agent::Role::System => "system",
                bamboo_core::agent::Role::User => "user",
                bamboo_core::agent::Role::Assistant => "assistant",
                bamboo_core::agent::Role::Tool => "tool",
            };
...
impl LLMProvider for CopilotProvider {
    async fn chat_stream(
        ...
        log::debug!("Sending request to Copilot API with {} messages and {} tools", 
        ...
        if !response.status().is_success() {
            ...
            if status == 401 || status == 403 {
                return Err(LLMError::Auth(
                    format!("Authentication failed: {}. Please run authenticate() again.", text)
                ));
            }
        ...
        let stream = response
            ...
            .map(|event| {
                ...
                let chunk: CopilotChunk = match serde_json::from_str(&event.data) {
                    Ok(c) => c,
                    Err(e) => {
                        log::warn!("Failed to parse Copilot chunk: {} - data: {}", e, event.data);
...
mod tests {
    ...
    fn test_convert_messages() {
        let provider = CopilotProvider::new();
        let messages = vec![
            bamboo_core::Message::system("You are helpful"),
            bamboo_core::Message::user("Hello"),
        ];
