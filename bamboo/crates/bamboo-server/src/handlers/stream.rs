...
pub async fn handler(
    ...
    // 创建 Agent 事件通道
    let (event_tx, event_rx) = mpsc::channel::<bamboo_core::AgentEvent>(100);
    
    // 包装 spawn_sse_sender 以添加 debug 日志
    let (debug_event_tx, mut debug_event_rx) = mpsc::channel::<bamboo_core::AgentEvent>(100);
    ...
    let event_stats = tokio::spawn({
        ...
        async move {
            ...
            while let Some(event) = debug_event_rx.recv().await {
                ...
                match &event {
                    bamboo_core::AgentEvent::Token { content } => {
    ...
    tokio::spawn(async move {
        ...
        let initial_message = session.messages.last()
            .filter(|m| matches!(m.role, bamboo_core::agent::Role::User))
        ...
        if !initial_message.is_empty() {
            ...
            if let Err(e) = &result {
                ...
                let _ = debug_event_tx.send(bamboo_core::AgentEvent::Error {
                    message: e.to_string(),
                }).await;
            } else {
