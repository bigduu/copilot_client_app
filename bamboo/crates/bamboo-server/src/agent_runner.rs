...
use bamboo_core::{Session, Message, AgentEvent, AgentError};
use bamboo_core::agent::events::TokenUsage;
use bamboo_core::tools::{ToolExecutor, ToolCall};
use bamboo_llm::{LLMProvider, LLMChunk};
...
pub async fn run_agent_loop_with_config(
    ...
    if let Some(ref system_prompt) = config.system_prompt {
        if !session.messages.iter().any(|m| matches!(m.role, bamboo_core::Role::System)) {
