#!/bin/bash
# Simplified E2E Test for Bamboo

set -e

echo "=========================================="
echo "Bamboo Simplified E2E Test"
echo "=========================================="
...
cd "$PROJECT_ROOT"
~/.cargo/bin/cargo build -p bamboo-server -p bamboo-cli --quiet 2>&1 | tail -5
echo -e "${GREEN}✓ Build complete${NC}"

echo ""
echo "[2/6] Starting server..."
DEBUG=true "$PROJECT_ROOT/target/debug/bamboo-server" --port 8081 &
SERVER_PID=$!
...
"$PROJECT_ROOT/target/debug/bamboo-cli" --help > /dev/null 2>&1 && echo -e "${GREEN}✓ CLI working${NC}" || echo -e "${RED}✗ CLI failed${NC}"
