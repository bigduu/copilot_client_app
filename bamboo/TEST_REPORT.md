...
### 1. 单元测试
- **文件**: `bamboo/crates/bamboo-core/src/lib_tests.rs`
- **描述**: 核心类型测试（Session, Message, Tool 等）
- **数量**: 8 个测试

### 2. Mock 测试
- **文件**: `bamboo/crates/bamboo-llm/tests/mock_provider.rs`
- **描述**: MockLLMProvider 实现和测试
- **数量**: 3 个测试
- **用途**: 用于测试 Agent Loop 而不依赖 OpenAI API

### 3. 集成测试
- **文件**: `bamboo/crates/bamboo-server/tests/integration_test.rs`
- **描述**: HTTP API 端点测试
- **数量**: 5 个测试
- **框架**: `actix_web::test`
...
```rust
use bamboo_llm::tests::mock_provider::MockLLMProvider;
