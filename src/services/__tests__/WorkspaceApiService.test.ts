...
describe('WorkspaceApiService', () => {
  ...
  describe('validateWorkspacePath', () => {
    ...
    it('should handle validation errors', async () => {
      ...
      await expect(workspaceApiService.validateWorkspacePath('/invalid/path')).rejects.toThrow(
        'HTTP 400: Invalid path'
      );
    });
  });
    ...
    it('should get path suggestions', async () => {
      const mockSuggestions = {
        suggestions: [
          ...
          { path: '/documents', name: 'Documents', suggestion_type: 'documents' },
        ],
      };
