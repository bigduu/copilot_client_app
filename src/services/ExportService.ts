...
export class ExportService {
  ...
  private static async saveFile(
    ...
    const filters =
      format === "markdown"
        ? FileOperationsService.FILTERS.MARKDOWN.map((f) => ({
            name: f.name,
            extensions: Array.from(f.extensions),
          }))
        : FileOperationsService.FILTERS.PDF.map((f) => ({
            name: f.name,
            extensions: Array.from(f.extensions),
          }));
  ...
  private static generateFilename(chatId: string, format: string): string {
    ...
    return FileOperationsService.generateChatExportFilename(
      chatId,
      extension as "md" | "pdf",
    );
  }
