...
fn test_performance_long_conversation() {
    ...
    for i in 0..500 {
        ...
        context.handle_event(ChatEvent::LLMResponseCompleted);
    ...
    println!("  Avg time per message: {:?}", duration / message_count as u32);
    ...
    assert!(duration.as_secs() < 5, "Long conversation took too long: {:?}", duration);
...
fn test_performance_concurrent_contexts() {
    ...
    for context_id in 0..10 {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in 0..50 {
                context.handle_event(ChatEvent::UserMessageSent);
                add_user_message(&mut context, &format!("Context {} message {}", context_id, i));
            ...
            results_clone.lock().unwrap().push((context_id, message_count, thread_duration));
        });
    ...
    for (context_id, message_count, duration) in results.iter() {
        println!("  Context {}: {} messages in {:?}", context_id, message_count, duration);
        assert_eq!(*message_count, 100);
    }
    
    // Performance assertion: concurrent operations should complete in reasonable time
    assert!(total_duration.as_secs() < 10, "Concurrent operations took too long: {:?}", total_duration);
...
fn test_performance_tool_intensive() {
    ...
    assert!(duration.as_secs() < 3, "Tool-intensive scenario took too long: {:?}", duration);
...
fn test_performance_streaming() {
    ...
    for response_num in 0..50 {
        ...
        if (response_num + 1) % 10 == 0 {
            println!("  Progress: {} streaming responses completed", response_num + 1);
        }
    }
    ...
    // Performance assertion
    assert!(duration.as_secs() < 5, "Streaming took too long: {:?}", duration);
