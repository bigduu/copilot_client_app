//! Mermaid Enhancement Enhancer
//!
//! Injects Mermaid diagram guidelines into the system prompt when enabled.
...
/// Mermaid Enhancement Enhancer
///
/// Injects Mermaid diagram guidelines into the system prompt when the
/// `mermaid_diagrams` configuration flag is enabled.
///
/// This encourages the AI to use visual representations (Mermaid diagrams)
/// when explaining concepts, processes, relationships, or data structures.
///
/// # Priority
///
/// This enhancer uses priority 50, placing it after role context (90)
/// and tool definitions (60), but before general context hints (40).
pub struct MermaidEnhancementEnhancer;

impl MermaidEnhancementEnhancer {
    /// Create a new Mermaid enhancement enhancer
    pub fn new() -> Self {
        Self
    }

    /// Get the Mermaid enhancement prompt content
    fn get_mermaid_prompt(&self) -> String {
        String::from(
            r#"
...
impl Default for MermaidEnhancementEnhancer {
    fn default() -> Self {
        Self::new()
    }
}

impl PromptEnhancer for MermaidEnhancementEnhancer {
    fn name(&self) -> &str {
        "mermaid_enhancement"
    }

    fn enhance(&self, ctx: &ProcessingContext) -> Option<PromptFragment> {
        // Check if mermaid_diagrams is enabled in config
        if !ctx.chat_context.config.mermaid_diagrams {
            log::debug!("[MermaidEnhancementEnhancer] Mermaid enhancement disabled, skipping");
            return None;
        }

        log::debug!(
            "[MermaidEnhancementEnhancer] Adding Mermaid guidelines to prompt (priority: 50)"
        );

        Some(PromptFragment {
            content: self.get_mermaid_prompt(),
            source: "mermaid_enhancement".to_string(),
            priority: 50,
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_mermaid_enhancement_enabled() {
        ...
        assert_eq!(fragment.source, "mermaid_enhancement");
        assert_eq!(fragment.priority, 50);
        assert!(
            fragment
                .content
                .contains("Visual Representation Guidelines")
        );
        assert!(fragment.content.contains("Mermaid diagrams"));
    }

    #[test]
    fn test_mermaid_enhancement_disabled() {
        let message = create_test_message();
