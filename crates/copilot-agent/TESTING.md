...
# Debug 构建（快速）
cargo build -p copilot-agent-server -p copilot-agent-cli

# Release 构建（优化）
cargo build -p copilot-agent-server -p copilot-agent-cli --release
```
...
```bash
# 基本启动
cargo run -p copilot-agent-server
...
预期输出：
```
[2026-02-01 04:29:30.208] INFO [copilot_agent_server] - Starting Copilot Agent Server on port 8080
...
# 查看历史
cargo run -p copilot-agent-cli -- history --session-id {session_id}
```
...
```
[2026-02-01 04:29:30.208] INFO [copilot_agent_server] - Starting Copilot Agent Server on port 8080
[2026-02-01 04:29:30.209] DEBUG [copilot_agent_server] - Debug mode enabled
[2026-02-01 04:29:30.209] DEBUG [copilot_agent_server] - Server configuration:
[2026-02-01 04:29:30.209] DEBUG [copilot_agent_server] -   Port: 8080
[2026-02-01 04:29:30.209] DEBUG [copilot_agent_server] -   Debug: true
```
...
- **Server 日志**: 标准输出 / `~/.copilot-agent/debug.log`
- **会话数据**: `~/.copilot-agent/{session_id}.json`

## 性能测试

```bash
# 使用 release 构建进行性能测试
cargo build --release -p copilot-agent-server
