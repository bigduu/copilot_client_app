...
use copilot_agent_core::tools::{ToolCall, ToolExecutor};
use copilot_agent_core::{AgentError, AgentEvent, Message, Session};
use copilot_agent_llm::{LLMChunk, LLMProvider};
use uuid::Uuid;
...
pub struct AgentLoopConfig {
    ...
    /// 如果为 true，跳过添加初始用户消息（消息已在 session 中）
...
pub async fn run_agent_loop_with_config(
    ...
    if let Some(ref system_prompt) = config.system_prompt {
        if !session
            .messages
            .iter()
            .any(|m| matches!(m.role, copilot_agent_core::Role::System))
        {
            session
                .messages
                .insert(0, Message::system(system_prompt.clone()));
            log::info!(
                "[{}] Added system prompt to session (length: {} chars)",
                session_id,
                system_prompt.len()
            );
    ...
    if !config.skip_initial_user_message {
        ...
        log::debug!(
            "[{}] Skipped adding user message (already in session), total messages: {}",
            session_id,
            session.messages.len()
        );
    }
    ...
    for round in 0..config.max_rounds {
        ...
        log::debug!(
            "[{}] Available tools: {} (base: {}, from skills: {})",
            session_id,
            tool_schemas.len(),
        ...
        let mut stream = match llm.chat_stream(&session.messages, &tool_schemas).await {
            Ok(s) => {
        ...
        if accumulated_tool_calls.is_empty() {
            ...
            // 添加 assistant 消息到历史
            session.add_message(Message::assistant(accumulated_content.clone(), None));

            log::debug!(
                "[{}] Added assistant message, content length: {}",
                session_id,
        ...
        for (idx, tool_call) in accumulated_tool_calls.iter().enumerate() {
            ...
            match tools.execute(tool_call).await {
                Ok(result) => {
                    ...
                    log::debug!(
                        "[{}] Tool {} completed in {}ms, success: {}, result length: {}",
                        session_id,
                        tool_call.function.name,
                        tool_duration,
                        result.success,
    ...
    log::debug!(
        "[{}] Agent loop completed, final message count: {}",
        session_id,
        session.messages.len()
    );
...
fn finalize_tool_calls(parts: Vec<PartialToolCall>) -> Vec<ToolCall> {
    parts
        ...
        .map(|p| {
            log::debug!(
                "Finalizing tool call: {} (args length: {})",
                p.name,
                p.arguments.len()
            );
            ToolCall {
...
mod tests {
    ...
    #[test]
    fn test_skip_initial_message_flag() {
        // 验证新配置标志可以正确设置
