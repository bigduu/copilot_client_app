...
use web_service::services::system_prompt_enhancer::{EnhancementConfig, SystemPromptEnhancer};
...
async fn test_enhance_prompt_with_tools() {
    ...
    let enhancer = SystemPromptEnhancer::new(tool_registry.clone(), EnhancementConfig::default());

    let base_prompt = "You are a helpful assistant.";
    let result = enhancer
        .enhance_prompt(base_prompt, &AgentRole::Actor)
        .await;
...
async fn test_enhance_prompt_without_tools() {
    ...
    let enhancer = SystemPromptEnhancer::new(tool_registry.clone(), config);
...
async fn test_enhance_prompt_with_different_roles() {
    ...
    let result_actor = enhancer
        .enhance_prompt(base_prompt, &AgentRole::Actor)
        .await;
    assert!(result_actor.is_ok());

    // Test with Planner role
    let result_planner = enhancer
        .enhance_prompt(base_prompt, &AgentRole::Planner)
        .await;
...
async fn test_multiple_concurrent_enhancements() {
    ...
    for i in 0..10 {
        ...
        join_set.spawn(async move {
            ...
            enhancer_clone
                .enhance_prompt(&prompt, &AgentRole::Actor)
                .await
        });
    }
