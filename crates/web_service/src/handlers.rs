...
pub async fn chat_completions(
    ...
    let internal_messages: Vec<copilot_client::model::stream_model::Message> =
...
async fn handle_stream_request(
    messages: Vec<copilot_client::model::stream_model::Message>,
    model: Option<String>,
    copilot_client: web::Data<Arc<CopilotClient>>,
) -> Result<HttpResponse> {
            ...
            match message {
                Ok(bytes) => {
                    ...
                    if let Ok(chunk_str) = String::from_utf8(bytes.to_vec()) {
                        if let Ok(internal_chunk) = serde_json::from_str::<copilot_client::model::stream_model::StreamChunk>(&chunk_str) {
...
async fn handle_non_stream_request(
    ...
    while let Some(message) = rx.recv().await {
        match message {
            ...
            Err(e) => {
                error!("Stream error: {}", e);
                return Ok(create_error_response(&e, "Failed to get response"));
            }
        }
    }

    // Ensure the handle completes
    if let Err(e) = handle.await {
        error!("Stream handle error: {}", e);
        return Ok(create_error_response(
            &anyhow::anyhow!(e),
            "Stream handle failed",
        ));
    }
    ...
    let response = OpenAIChatCompletionResponse {
        ...
        choices: vec![OpenAIChoice {
            ...
            message: Some(OpenAIMessage {
                ...
                content: OpenAIContent::Text(full_content),
            }),
            delta: None,
            finish_reason,
        }],
