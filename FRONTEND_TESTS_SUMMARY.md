...
**Note**: The 3 failed e2e tests are Playwright tests that require `@playwright/test` package, which is not related to our file reference logic tests. The 1 failed test in `useChatManager.test.ts` is also unrelated to file reference functionality.
